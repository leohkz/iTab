...
type AppGridProps = {
  ...
  onPageStateChange?: (current: number, total: number) => void;
...
// ── GSAP page-slide animation ─────────────────────────────────────────────
function useGsapPageTransition(
  containerRef: React.RefObject<HTMLDivElement | null>,
  slideDir: 'left' | 'right' | null,
  animating: boolean,
) {
  const prevDir = useRef<'left' | 'right' | null>(null);

  useEffect(() => {
    if (!animating || !slideDir || !containerRef.current) return;
    if (prevDir.current === slideDir) return;
    prevDir.current = slideDir;
  ...
  }, [animating, slideDir, containerRef]);
}

export function AppGrid({
  apps, folders, editing, selectedFolderId, gridColumns, gridRows: _gridRows,
  currentSpaceId, spaces, spaceDirection, pendingNavigatePage, onNavigated, onPageChange,
  onPageStateChange,
  t,
  ...
  const goToPage = useCallback((page: number, dir?: 'left' | 'right') => {
    ...
    const resolvedDir = dir ?? (page > currentPage ? 'right' : 'left');
